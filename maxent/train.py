...
def idx_demo(env, one_feature):
    ...
    env_distance = (env_high - env_low) / one_feature   # for distance from high and low abslute value and one_feature=20
    raw_demo = np.load(file="expert_demo/expert_demo.npy",allow_pickle=True)
    demonstrations = np.zeros((len(raw_demo), len(raw_demo[0]), 3)) #(20, 130, 3)
